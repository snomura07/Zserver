...
int ZmqWrapper::pollMessage(std::string &msg, int timeout, std::string topic)
{
    zmq_pollitem_t item = { sessionMap[topic].socket, 0, ZMQ_POLLIN, 0 };
    auto res = zmq_poll (&item, 1, timeout);
    ...
    if (item.revents & ZMQ_POLLIN) {
        ...
        if (delimiter_pos != std::string::npos) {
            ...
            if (sessionMap[topic].callback) {
                sessionMap[topic].callback(msg, topic);
            }
        }
        else {
            std::cerr << "Delimiter not found in received message." << std::endl;
